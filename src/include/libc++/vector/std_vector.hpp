...
template <typename T>
std::vector<T>::vector(uint64_t size, const T &data): m_size(size), m_cap(2*size+1), m_data(new T[2*size+1])
{
    for (T &element: *this)
        element = data;
};
...
template <typename T>
std::vector<T>::vector(const vector<T> &other)
{
    if (m_data)
        delete[] m_data;
...
template <typename T>
std::vector<T>::vector(vector<T> &&other)
{
    m_size = other.m_size;
    m_cap = other.m_cap;
    m_data = other.m_data;
...
template <typename T>
void
std::vector<T>::reserve(uint64_t cap)
{
    uint64_t smallest = std::min(m_cap, cap);
...
template <typename T>
void
std::vector<T>::resize(uint64_t size)
{
    if (size > m_cap)
        reserve(size);
    m_size = size;
...
template <typename T>
void
std::vector<T>::push(const T &data)
{
    if (m_size+1 >= m_cap)
        reserve(2*m_cap);
...
template <typename T>
std::vector<T> &
std::vector<T>::operator = (const vector &other)
{
    if (this != &other)
    {
        if (m_data)
...
template <typename T>
std::vector<T> &
std::vector<T>::operator = (vector &&other) noexcept
{
    if (this == &other)
        return *this;
